...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # --- VIDEO outputs (VHS_VideoCombine -> "gifs"): upload p/ R2/S3 ou base64 ---
            if "gifs" in node_output:
                ...
                for vinfo in node_output["gifs"]:
                    vfilename = vinfo.get("filename")
                    vsubfolder = vinfo.get("subfolder", "")
                    vtype = vinfo.get("type")
                    if not vfilename:
                        continue
                    vbytes = get_image_data(vfilename, vsubfolder, vtype)
                    if not vbytes:
                        errors.append(f"Failed to fetch video data for {vfilename}")
                        continue
                    endpoint = os.environ.get("BUCKET_ENDPOINT_URL")
                    if endpoint:
                        try:
                            u = _urlparse(endpoint)
                            base_endpoint = f"{u.scheme}://{u.netloc}"
                            bucket = u.path.strip("/").split("/")[0]
                            s3 = boto3.client(
                                "s3",
                            ...
                            key = f"{job_id}/{vfilename}"
                            s3.put_object(Bucket=bucket, Key=key, Body=vbytes, ContentType="video/mp4")
                            url = f"{base_endpoint}/{bucket}/{key}"
                            print(f"worker-comfyui - Uploaded video {vfilename} to R2: {url}")
                            output_data.append({"filename": vfilename, "type": "s3_url", "data": url})
                        except Exception as e:
                            errors.append(f"Error uploading video {vfilename} to R2: {e}")
                    else:
                        output_data.append({
                            "filename": vfilename,
                            "type": "base64",
                            "data": base64.b64encode(vbytes).decode("utf-8"),
                        })
